...
We used a Hidden Markov Model (HMM) [REF] refinement scheme to assess and improve the parameter choice for FLC.
First, we calculated FLC for various combinations of $min\_samples$ and $\varepsilon$.
We scanned $min\_samples$ from 5 to 60 in steps of 1 and $\varepsilon$ from 1 nm to $r_{diagonal}$ in an increament of 0.05 nm.
Where $r_{diagonal}$ is the half of periodic box diagonal distance in $xy$ plane.
...
On the FLC evolution data corresponding to each trajectory, we fit an HMM model using the hmmlearn python module [REF].
...
Hence, the right answer to the number of hidden states that best fit the model is unknown.
Therefore, we used a brute force approach to fitting the HMM model with N states, where N = 1 to 5.
We checked how the model converges for each value of N by running fitting from 10 random starting states for HMM.
We chose the N that converges best as the optimum number of states for fitting.
Once we choose the best number of states, HMM fitting is done on the trajectory FLC data.
This gives a hidden state sequence for the corresponding trajectory.
Now, we take the FLC difference between extreme hidden states.
...
This cutoff choice is arbitrary and just enough to filter out parameter pairs that distinguish states clearly from each other.
(2) From the filtered dataset, check for the occurrence of a specific pair of $(min\_samples, \varepsilon)$ across the data sets corresponding to all the temperature replicas.
...
Now, instead of a temperature-specific $min\_samples$ and $\varepsilon$ for each species in a given ternary mixture, we have a unique pair for each lipid species in a system.
...
AJP and AG designed the project. AJP performed the research. AJP and AG wrote manuscript.
