...
mixed and separated lipid states is hard to achieve in conventional MD simulations. Here, we propose a proof-of-concept pipeline that can realize such multiple-state transitions by combining coarse-grained model membranes with enhanced sampling protocols.
...
Here we present a novel proof-of-concept pipeline to realize multiple mixing and separating events in a lipid bilayer system and estimate the equilibrium free energy.
...
Interestingly, for relatively slow separating DPPC-DIPC-CHOL, CEI and $\text{SI}_{\text{noCHOL}}$ appear to indicate a slower transition than FLC, indicating that while these variables are correlated, they do not measure precisely the same phenomenon.
...
As shown in Fig \ref{figs4:view}A, for the test system DPPC-DIPC-CHOL replica at 323K, the free energy curves do not agree with each other despite the number of WE iterations used to construct them being identical.
From CEI free energy curves, we see both states (mixed and separated) are equally likely which is inconsistant with the spontaneous separation seen in the standard MD.
The $\text{SI}_{\text{noCHOL}}$ curves suggesting a mixed state is more likely than a separated state has an even worse problem.
...
The disagreement of free energy curves as a function of WE iteration blocks used to generate them suggests that the underlying sampling has not converged.
On the other hand, the FLC free energy curve shows that the separated state is favorable, but the free energy barrier between the two states is small.
...
Interestingly, if we monitor the evolution of configurational distributions of each variable as a function of WE iterations, Fig 4B appears to have converged or at least equilibrated.

These conflicting results raise the following questions: Why do variables that make sense while tracking the system in standard CG MD simulation gives a different free energy landscape after WE simulation?
...
One clue to diagnose the problem lies in our starting states. In contrast to most WE calculations, which begin the simulation with all walkers in the “first” bin, we started with diverse states from across the whole range of the collective variable. 

Yet the configurational distribution of collective variables staying mostly undisturbed in two states (Fig \ref{figs4:view}B) and
...
The state bounds are chosen based on visual inspection of the corresponding free energy landscapes in Fig \ref{figs4:view}A.
The choice is arbitrary but reasonable enough to give us a picture of what is happening. 
Fig \ref{figs4:view}C shows the mean flux from mixed to separated state and vice versa in WE simulations driven using each collective variable.
Here, mean flux is calculated for a window of 10 WE iterations to avoid complicating the interpretation due to the WEED reweighting.
The peaks in these plots imply probability crossing states.
For CEI, no state crossing occurs in either direction during WE simulation.
...
As a result the free energy curves compiled with these methods simply reflect local relaxation of their state distribution.
Meanwhile, for FLC, flux into and out of the states is relatively high and more immediate than the other two variables.
...
It is interesting to note that, For CEI, we see a significant drop in the population of mixed state during iterations 290-300, without a concomitant rise in the other state.
Since our state definition we chose does not span the entire collective variable space leaving a room for the barrier that separates two states, this indicates that some walkers transitioned out of mixed state , but remained trapped in the middle of the collective variable and never transitioned to separated state.

The walkers crossing basins and thereby constituting a flow of probability between states is crucial for the success of a WE equilibrium simulation\cite{Zuckerman2017}.
Thus these results suggest that for CEI and $\text{SI}_{\text{noCHOL}}$, the initial distribution of walkers is not relaxed throughout the WE simulation.
...
This explains why in free energy landscapes associated with CEI and $\text{SI}_{\text{noCHOL}}$, we do not see converged free energy curves.
However, with FLC, we see good state crossing and relatively well-converged free energy curves. 
...
But to showcase the capability of the pipeline we have presented here, we define an ad hoc cutoff, FLC = 0.525 and 0.625, for DPPC-DIPC-CHOL and DPPC-DAPC-CHOL  lipid systems respectively based on visual inspection.   
