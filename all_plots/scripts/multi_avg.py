...
fig, axs = plt.subplots(nrows=1, ncols=1, sharex=True, sharey=True, figsize=(7,8))
...
for l in range(len(strided_legend)):
    ...
    FileName = str(args.temp) + "K/multi_west/" + args.cv + "_average_" + str(strided_legend[l])
    ...
    for j in range(len(sorted_dat_files)):
        ...
        if FileName in sorted_dat_files[j]:
            ...
            axs.plot(midpoints,plothist,label=str(strided_legend[l]))
