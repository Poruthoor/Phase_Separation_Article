...
%DIF DEL FIRST-SUB.tex   Fri Aug 25 17:55:23 2023
%DIF ADD main.tex        Mon Aug 28 12:39:26 2023
...
%DIF 20c21
%DIF < \affil[1]{University of Rochester Medical Center, Rochester, NY 14620}
%DIF -------
...
Moreover, lipid domains are conserved throughout the tree of life, implying their relevance in regulating cellular processes\cite{Sezgin2017}\DIFaddbegin \DIFadd{; for example, recent work from the Keller lab showed that yeast actively regulate the composition of their vacuolar membranes to keep the melting temperature well above the growth temperature of the yeast\mbox{%DIFAUXCMD
\cite{Keller2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
For these reasons, understanding the thermodynamics of lipid phase separation as a function of the composition of the system is a critical step toward understanding the  \DIFaddbegin \DIFadd{individual lipid properties that underlie the formation of }\DIFaddend domains and the resultant functional modulations. 
...
Hence, simple model membranes have been extensively used to mimic the phase separation of complex cell membrane  in vitro\cite{Veatch2003a,Veatch2002,Veatch2003} and in silico\cite{Risselada2008,Lin2016,Lin2019}.
Such studies provide powerful insights into phase-separated domains, their properties, and whether the process is favorable, but as a rule  \DIFaddbegin \DIFadd{atomistic and coarse-grained models }\DIFaddend cannot quantify the underlying thermodynamics\DIFaddbegin \DIFadd{, although simpler lattice-based models can \mbox{%DIFAUXCMD
\cite{Schick1990,Schick2006,Schick2015,Schick2017,Schick2020,Almeida2012,Biltonen1996,Biltonen2001,Javanainen2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend . This limits our ability to assess the contributions of different mechanistic effects to the process.
...
The  \DIFaddbegin \DIFadd{clustering }\DIFaddend is defined using the Density-Based Spatial Clustering of Applications with Noise (DBSCAN) algorithm \cite{MartinEsterHans-PeterKriegelJiirgSander1996, Ester2017} as implemented in scikit-learn \cite{PedregosaF.VaroquauxG.GramfortA.MichelV.ThirionB.GriselO.BlondelM.PrettenhoferP.WeissR.andDubourgV.VanderplasJ.PassosA.CournapeauD.BrucherM.PerrotM.Duchesnay2011}.
For lipid DBSCAN clustering,  \DIFaddbegin \DIFadd{we supply a }\DIFaddend 2-dimensional  \DIFaddbegin \DIFadd{lipid-lipid distance matrix that accounts }\DIFaddend for periodic boundary conditions; the distance calculation was performed using LOOS, with the two leaflets treated independently.
...
Since lipid motion in a bilayer is constrained primarily on a plane and MARTINI beads for a lipid are of similar radii, we  \DIFaddbegin \DIFadd{used }\DIFaddend the two-dimensional version of Kepler's conjecture that the densest packing of unit disks in a plane is hexagonal close packing (Thue's Theorem) \DIFaddbegin \DIFadd{, and
}\DIFaddend chose 7 (6 nearest neighbors + 1 central lipid) as $min\_samples$ for all the lipid species.
However, $\varepsilon$ was chosen differently for each lipid species based on their first  \DIFaddbegin \DIFadd{nearest-neighbor }\DIFaddend distance from the central lipid.
We used the $xy\_rdf$ tool in LOOS to calculate an individual lipid species' first  \DIFaddbegin \DIFadd{nearest-neighbor }\DIFaddend distance.
...
When lipids segregate into phases, one result is that the local concentration of a given lipid type in the vicinity of other lipids of the same type is increased. We attempted to exploit this phenomenon by tracking the degree of local lipid enrichment \DIFaddbegin \DIFadd{; this was inspired by previous work from our lab \mbox{%DIFAUXCMD
\cite{Grossfield2022}}\hskip0pt%DIFAUXCMD
, although other groups used similar methods }\DIFaddend \cite{Gu2019, Gu2020}.
...
\end{equation}
 ...
 \DIFaddbegin \noindent \DIFadd{where }\DIFaddend $X_{ij}$ denotes $j^{th}$ lipid of $X_i$ lipid species.
...
\DIFaddbegin \subsection*{\DIFadd{Optimized version of FLC}}

\DIFadd{We used a Hidden Markov Model (HMM) \mbox{%DIFAUXCMD
\cite{hmmlearn} }\hskip0pt%DIFAUXCMD
refinement scheme to optimize the parameters for FLC in order to improve its ability to discriminate between states.
...
where $r_{diagonal}$ is half of the periodic box diagonal distance in the plane of the membrane.
...
On the FLC evolution data corresponding to each trajectory, we fit an HMM model using the hmmlearn python module \mbox{%DIFAUXCMD
\cite{hmmlearn}}\hskip0pt%DIFAUXCMD
.
...
\DIFadd{To arrive at an optimized FLC parameter set, we filtered this larger dataset using the following steps:
...
Now, instead of a temperature-specific $min\_samples$ and $\varepsilon$ for each species in a given ternary mixture, we have a unique pair for each lipid species in a system. We refer to the coordinate using these parameters as FLC$_{\text{opt}}$. For the DIPC system, the optimized values ($min\_samples$, $\varepsilon$) are (26.5 }{\DIFadd{\AA}}\DIFadd{, 24), (31.5 }{\DIFadd{\AA}}\DIFadd{, 26), and (32.5 }{\DIFadd{\AA}}\DIFadd{, 23) for DPPC, DIPC, and cholesterol respective. For the DAPC system, they are (33.0 }{\DIFadd{\AA}}\DIFadd{, 42), (44.5 }{\DIFadd{\AA}}\DIFadd{, 56), and (50.0 }{\DIFadd{\AA}}\DIFadd{, 41) for DPPC, DAPC, and cholesterol. FLC$_{\text{opt}}$ was not used for WE sampling, only as a coordinate on which to project the free energy.
}

\DIFaddend \subsection*{Weighted Ensemble Simulation}
\DIFaddbegin \subsubsection*{\DIFadd{Introduction to weighted ensemble simulations}}
\DIFaddend 

\DIFaddbegin \DIFadd{Although the WE method has been reviewed extensively elsewhere\mbox{%DIFAUXCMD
\cite{Zwier2015, Zuckerman2017}}\hskip0pt%DIFAUXCMD
, we provide a brief overview here. The essence of WE is to generate a large number of short unbiased trajectories, such that they evenly cover conformation space as quantified by a chosen collective variable (CV) or progress coordinate. Specifically, a coordinate is chosen and broken into bins. Short MD trajectories (called walkers) are run, and the location of the final state identified. If a given bin has more than a chosen target number of walkers, some of those walkers are culled and their statistical weight is distributed among the remaining walkers in the bin. If there are fewer than the target number of walkers in a bin (e.g. because more walkers departed than arrived, or because a walker moved into a previously unpopulated bin), the walker is split to produce the target number and its statistical weight evenly distributed among the new  walkers.
}

\DIFadd{The WE approach has several major advantages. First, it can be proved that WE converges to the correct free energy distribution in the good-sampling limit \mbox{%DIFAUXCMD
\cite{Zuckerman2017}}\hskip0pt%DIFAUXCMD
. Second, its use of a large number of short trajectories is readily parallelizable, making it efficient on high-performance computing resources. Third, it directly produces unbiased estimates of the kinetics that are independent of the chosen coordinate (although some coordinates may converge faster). Fourth, unlike almost every other enhanced sampling method, the MD itself is unbiased; the collective variable is computed only after the MD has finished. This last property was most important to the present application; the coordinates we used are computationally expensive, particularly if a plugin such as PLUMED\mbox{%DIFAUXCMD
\cite{Tribello2019} }\hskip0pt%DIFAUXCMD
or COLVARS\mbox{%DIFAUXCMD
\cite{Fiorin2013} }\hskip0pt%DIFAUXCMD
were used, since both are restricted to a single CPU core and are not GPU-accelerated.
}

\DIFaddend \subsubsection*{Preparing seeding configurations for WE simulation}
...
Figure \ref{figs5:view} shows the free energy profiles of \DIFaddbegin \DIFadd{the }\DIFaddend DPPC-DAPC-CHOL, DPPC-DIPC-CHOL, and DPPC-POPC-CHOL lipid bilayer systems.
Each curve is generated by averaging four WE replica simulations, as described in the Methods section, using the last 10 iterations (491-500) of each replica.
 \DIFaddbegin \DIFadd{The variance between the }\DIFaddend individual replica free energy curves  \DIFaddbegin \DIFadd{is generally quite small, approximately 0.5 kcal/mol, shown }\DIFaddend in Supplementary Figure S7.
The DPPC-DAPC-CHOL system has a double well behavior at 323K and 353K.
However, both basins correspond to  \DIFaddbegin \DIFadd{high FLC, such that the lower FLC basin is still extremely non-ideally mixed. Based }\DIFaddend on visual inspection \DIFaddbegin \DIFadd{, it seems the primary difference is the degree to which lipids have coalesced into domains.
Indeed, even }\DIFaddend at 423 K, we see that this system prefers to be in configurations where more than 60\% of lipids are in clusters.
...
\begin{figure}[hbt!]
    \centering
     \DIFaddbeginFL \includegraphics[width=3.25in]{Figures/Main/6/placeholder_v0.jpg}
    \DIFaddendFL \caption{A. Free energy landscapes reconstructed using the ensemble of configurations generated during last 10 iterations of FLC-driven WE simulation. The free energy landscape is reconstructed with respect to $\text{SI}_{\text{noCHOL}}$ for DPPC-DAPC-CHOL and DPPC-DIPC-CHOL system respectively. B. The free energy landscape is reconstructed with respect to CEI for DPPC-DAPC-CHOL and DPPC-DIPC-CHOL system respectively. \DIFaddbeginFL \DIFaddFL{C. }\DIFaddendFL The \DIFaddbeginFL \DIFaddFL{free energy landscape is reconstructed with respect to FLC$_{\text{opt}}$ for DPPC-DAPC-CHOL and DPPC-DIPC-CHOL system respectively. In all panels, the }\DIFaddendFL lines are colored to represent different simulation temperatures \DIFaddbeginFL \DIFaddFL{.
    }\DIFaddendFL \label{figs6:view}\DIFaddbeginFL }
\DIFaddendFL \end{figure}

One outcome of WE simulation is the  ensemble of diverse structures generated by the trajectories.
By construction, WE resampling ensures that the weights associated with these walkers are \DIFaddbegin \DIFadd{known and }\DIFaddend unbiased.
Thus, we can reuse these weights to examine other variables of choice post-simulation without any need to reweight; this is advantageous, because reweighting  \DIFaddbegin \DIFadd{nearly always introduces significant noise due to heavy weights applied to the lowest $\Delta U$ structures}\DIFaddend .
...
Fig \ref{figs6:view}  \DIFaddbegin \DIFadd{shows }\DIFaddend reconstructed free energy landscapes  \DIFaddbegin \DIFadd{using 3 coordinates, each constructed using the structures and weights from last 10 iterations of }\DIFaddend each  \DIFaddbegin \DIFadd{FLC-generated replica; Panel A shows the results for $\text{SI}_{\text{noCHOL}}$, Panel B shows the results for CEI, and Panel C shows the results projected onto FLC$_{\text{opt}}$.
...
\subsection*{Computing the free energy of \DIFaddbegin \DIFadd{phase }\DIFaddend separation}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/Main/7/placeholder.jpg}
    \caption{$\Delta\Delta$G profile for DPPC-DAPC-CHOL and DPPC-DIPC-CHOL systems for a given cutoff. Each curve represents $\Delta\Delta$G for the system to transition from mixed to the separated state. Negative values indicate phase coexistence is favored\DIFaddbeginFL \DIFaddFL{. Figure S8 shows the sensitivity of the computed $\Delta\Delta$G to the choice of cutoff.}\DIFaddendFL }
    \label{figs7:view}
\end{figure}

If we define a cutoff FLC value that separates the  \DIFaddbegin \DIFadd{mixed }\DIFaddend and phase-separated states, we can compute the free energy change for phase separation as
\begin{equation}
    \Delta \Delta G_{\text{sep}} = -k_B T \ln \frac{p_{\text{sep}}}{p_{\text{mixed}}}
    \DIFaddbegin \label{eq:ddg}
\DIFaddend \end{equation}
\DIFaddbegin \noindent \DIFadd{where the probabilities are extracted by summing the weights from the weighted ensemble simulations.
}\DIFaddend 

One challenge, however, is that the locations of the basins shift with temperature, as seen in Fig \ref{figs5:view}A, and
at higher temperatures, the notion of two states  \DIFaddbegin \DIFadd{--- }\DIFaddend mixed and separated  \DIFaddbegin \DIFadd{--- }\DIFaddend breaks down as the double-well behavior of free energy curves turns into a single well.
...
For this reason, devising a rigorous systematic way to define the separator between the states will require more investigation.  \DIFaddbegin \DIFadd{As a first attempt}\DIFaddend , we use visual inspection to define FLC cutoffs of 0.525 and 0.625 for the DPPC-DIPC-CHOL and DPPC-DAPC-CHOL systems respectively\DIFaddbegin \DIFadd{; below those values, the systems are relatively well-mixed}\DIFaddend .
