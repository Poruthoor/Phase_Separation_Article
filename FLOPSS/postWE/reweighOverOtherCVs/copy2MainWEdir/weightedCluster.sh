...
then
    ...
    python3 ${HEADDIR}/CVs/wDBSCANanalysisSpecies.py \
        --model ${model} \
        --traj ${traj} \
        --lipid_list ${lipidList} \
        --r ${rFile} >> $TEMP_SYSTEM || exit 1

    cat $TEMP_SYSTEM | tail -n +4 | awk {'print $1,$4'} > ${fileName}_DPPC.dat
    cat $TEMP_SYSTEM | tail -n +4 | awk {'print $2,$5'} > ${fileName}_DXPC.dat
    cat $TEMP_SYSTEM | tail -n +4 | awk {'print $3,$6'} > ${fileName}_CHOL.dat
