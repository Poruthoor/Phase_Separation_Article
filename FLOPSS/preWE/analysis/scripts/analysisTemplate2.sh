...
###############################################################################
#                                                                             #
# Note : This bash script considers NO CHOLESTEROL . i.e, lipidlist and       #
#        radiusCutoff files have NO chloesterol in it                         #
# Usage :                                                                     #
#   bash analysisTemplate2.sh analysisPythonScriptFileName                    #  
#                                                                             #
...
do
    rm -rf ${WORKDIR}/analysis/data/${SYSTEM}/*/*/${PYTHON_PREFIX}_NO_CHOL.*
    list=${WORKDIR}/xy_rdf/${SYSTEM}/lipidList_NO_CHOL.dat

    for i in 298K 323K 423K 450K
    do
        rFile=${WORKDIR}/xy_rdf/${SYSTEM}/${i}/avgRcutoff_NO_CHOL.dat
       ...
       do
            ...
            then
                ...
                python3 ${WORKDIR}/CVs/${PYTHON_PREFIX}.py --model ${model} \
                    --traj ${traj} \
                    --lipid_list ${list} \
                    --r ${rFile} >> ${PYTHON_PREFIX}_NO_CHOL.dat
            fi
