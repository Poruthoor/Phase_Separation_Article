...
python3 $WEST_SIM_ROOT/common_files/segregationIndex.py \
    --model $WEST_SIM_ROOT/bstates/model.psf \
    --traj traj.dcd \
    --lipid_list $WEST_SIM_ROOT/common_files/lipidList_NO_CHOL.dat \
    --r $WEST_SIM_ROOT/common_files/avgRcutoff_NO_CHOL.dat > $TEMP || exit 1
...
# Clean up
rm -f *.mdp *.ndx *.top ref_system_centered.pdb *.gro traj.dcd traj.pdb $TEMP
rm -rf toppar
