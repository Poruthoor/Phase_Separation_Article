...
numberofdim=1  # number of dimensions : Just one : Enrichment of A around A

binsperdim=[26]   # You will have prod(binsperdim)+numberofdim*(2+2*splitIsolated)+activetarget bins total

pcoordlength=2 # length of the pcoord

maxcap=[5.75]	#for each dimension enter the maximum number at which binning can occur, if you do not wish to have a cap use inf

mincap=[3.30]  #for each dimension enter the minimum number at which binning can occur, if you do not wish to have a cap use -inf

targetstate=[None]    #enter boundaries for target state or None if there is no target state in that dimension

targetstatedirection=[1]  #if your target state is meant to be greater that the starting pcoor use 1 or else use -1.This will be done for each dimension in your simulation. We are starting from left bin to right.
